    ...
    public static function uploadarticleimg($id,$folder1)
    {
            ...
            || ($_FILES2["file"]["type"] == "image/pjpeg")))
        {
            ...
            else
            {
                ...
                $ressult_brand = CArticle::searchArticle_byid($id);
                if($ressult_brand){
                    if (is_file($folder.strrchr($ressult_brand['article_img'],'/'))) {

                        unlink($folder.strrchr($ressult_brand['article_img'],'/'));
                    }
                    move_uploaded_file($tmpFilePath, $desFilePath);
                    $res = CImages::editarticalpic($folder1.$_FILES2["file"]["name"],$id);
                    if($res){
