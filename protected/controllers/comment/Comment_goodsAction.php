<?php
class Comment_goodsAction extends CAction
{
    public function run()
    {
        $action = $this->getId();
        ...
        if($is_delete)
        {
            $result = CComment::deleteGoods_comment($comment_id,$is_delete);
            echo $result;die;
        }
        if($article && $datetime)
        {
        ...
        }elseif(empty($article) && $datetime)
        {
            ...
            $article_comment = CComment::searchGoods_comment();
        }
        $count = count($article_comment);
        //echo '<pre>';
        //var_dump($article_comment);die;
        $this->controller->layout = false;
        $this->controller->render('comment_goods',array('article_comment'=>$article_comment,'count'=>$count));
    }
}