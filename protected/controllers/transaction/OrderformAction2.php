...
class OrderformAction2 extends CAction{
    public function run()
    {
        $action = $this->getId();
        ...
        if($where)
        {
            ...
            $order_arr = CTransaction::searchAllorder55($where,$page,$size=10);
            $count = CTransaction::countorder_wite($where);
        }
        else if($search_order)
        {
        	$where = "a.id=$search_order";
            $order_arr = CTransaction::searchAllorder55($where,$page,$size=2);
            $count = CTransaction::countorder_wite($where);
        }else if($start_time && $end_time)
        {
            $where = "a.status=2 AND '$start_time'<a.create_time AND a.create_time<'$end_time' AND b.status=1 AND a.is_delete=0";
            $order_arr = CTransaction::searchAllorder55($where,$page,$size=2);
            $count = CTransaction::countorder_wite($where);
        }
        else{
            $where = "a.`status`=2 AND b.`status`=1 AND a.is_delete=0";
        	$order_arr = CTransaction::searchAllorder55($where,$page,$size=10);
            $count = CTransaction::countorder_wite($where);
        }
        //var_dump($order_arr);
         ...
         {
             $order_detail_arr = CTransaction::search_order_sub_id($value['id'],$status=1);
