...
class OrderformAction4 extends CAction{
    public function run()
    {
        $action = $this->getId();
        ...
        if($where)
        {
            ...
            $order_arr = CTransaction::searchAllorder55($where,$page,$size);
        ...
        {
            $where = "a.status=2 AND '$start_time'<a.create_time AND a.create_time<'$end_time' AND b.status=2 AND a.is_delete=0";
            $order_arr = CTransaction::searchAllorder55($where,$page,$size);
            $count = CTransaction::countorder_wite($where);
        }
        else{
            $where = "a.`status`=2 AND b.`status`=2 AND a.is_delete=0";
            $order_arr = CTransaction::searchAllorder55($where,$page,$size);
            $count = CTransaction::countorder_wite($where);
        }
        foreach($order_arr as $key=>$value)
        {
        	$order_detail_arr = CTransaction::search_order_sub_id($value['id'],$status=2);
		...
		//发货
        if($logistics_num && $order_sub_id && $is_send && $order_id)
        {
            /*如果为0 则无子订单*/
            if($type == 0)
            {
