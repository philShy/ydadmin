...
class OrderformAction5 extends CAction{
    public function run()
    {
        $action = $this->getId();
        ...
        {
            $where = "a.status=2 AND '$start_time'<a.create_time AND a.create_time<'$end_time' AND b.status=3 AND a.is_delete=0";
            $order_arr = CTransaction::searchAllorder55($where,$page,$size);
            $count = CTransaction::countorder_wite($where);
        }
        else{
            $where = "a.`status`=2 AND b.`status`=3 AND a.is_delete=0";
            $order_arr = CTransaction::searchAllorder55($where,$page,$size);
            $count = CTransaction::countorder_wite($where);
        }
        foreach($order_arr as $key=>$value)
        {
        	$order_detail_arr = CTransaction::search_order_sub_id($value['id'],$status=3);
        ...
        if($logistics_num && $order_id && $is_send)
        {
        	...
        	}elseif ($type == 1)
        	{
                ...
                CAdmin::updateNotice($order_id,5);
        	}
