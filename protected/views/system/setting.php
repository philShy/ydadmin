...
<style>
    .jinzhi{background: #ccc;color:#fff;border-radius: 2px;padding:2px 3px;}
    a{display: inline-block;margin:5px 0 0 20px;background: #6fb3e0;color:#fff;border-radius: 2px;padding:2px 3px;}
</style>
<body>
        ...
        <div class="tabbable">
            ...
            <div class="tab-content">
                <div id="home" class="tab-pane active">
                    <div class="form-group"><label class="col-sm-1 control-label no-padding-right" for="form-field-1"><i>*</i>任务计划： </label>
                        <div>
                            <a <?php if(file_exists('./cron-run.txt')&&file_exists('./cron-switch.txt')){echo 'class=jinzhi';}?> href='javascript:void(0)' id='create'>创建</a>
                            <a class="anniu" href='/test/test' id='open'>开启</a>
                        <a <?php if(!file_exists('./cron-switch.txt')){echo 'class=jinzhi';}?> href='javascript:void(0)' id='down'>关闭</a></div>
                    </div>
...
<script>
    $(function(){
        var url = '/system/setting'
        //$('.jinzhi')click(function(){return false;})
        $('#down').click(function(){
            $.post(url,{state:1},function(data){
                if(data)
                {
                    layer.alert('已关闭！',function(){
        ...
        $('#create').click(function(){
            $.post(url,{state:2},function(data){
                if(data==1)
                {
                    layer.alert('已创建！',function(){
                        window.location.reload(true)
                    })

                }else{
                    layer.alert('创建失败！',function(){
                        window.location.reload(true)
                    })
                }
            })
