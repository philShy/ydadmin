        ...
        <div class="article_sort_list">
            <table class="table table-striped table-bordered table-hover" id="sample-table">
                ...
                <tr>
                    ...
                    <th width="80px" style="display: none">状态</th>
                    <th width="150px">操作</th>
                </tr>
                </thead>
                <tbody>
                <?php foreach($doc_category as $key=>$value):?>
                    <tr>
                        ...
                        <td style="display: none"><?php echo $value['is_delete']?></td>
                        <td class="td-manage">
...
<script>
                ...
                if(num>0){  return false;}
                    ...
                    $.post(url,{cate_name:cate_name,cate_sort:cate_sort,introduce:introduce},function(data){
                        if(data){
                            ...
                            window.location.href='/system/doc_category'
                        }
                    })
                    layer.close(index);
                }
    ...
    function member_edit(iid){
        var url = '/system/doc_category';
        if(iid){
            $.post(url,{doc_cateid:iid},function(data){
        ...
        layer.open({
            ...
            yes:function(index,layero){
                ...
                if(num>0){  return false;}
                    ...
                    var cate_sort = $('.cate-sort').val();
                    var introduce = $('.introduce').val();
                    $.post(url,{doc_cate_id:iid,cate_name:cate_name,cate_sort:cate_sort,introduce:introduce},function(data){
                        if(data){
                            layer.alert(
                                '修改成功！',
                                {title: '提示框', icon:1,},
