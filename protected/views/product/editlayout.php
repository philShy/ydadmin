...
foreach ($layout as $key=>$value){
	$category=json_decode($value['category'])->name;
	$categorys=json_decode($value['category'])->id;
 	$bb= json_decode($value['category'],true);
 	foreach ($bb['data'] as $keys=>$values){
 		$small_id[]=$values['id'];
 	}
    $smallcate=implode(',', $small_id);
    $gg=json_decode($value['goods'],true) ;
    if(!empty($gg))
    {
        foreach($gg as $ke=>$ve){
            $goods_id[]=$ve['skuid'];
        ...
        $goods=implode(',',$goods_id);
    }

    $dd=json_decode($value['goods_advert'],true);
    $blb=json_decode($value['brand'],true);
    foreach($blb as $kee=>$vel){
        $brand_id[]=$vel['id'];
    };
    $brand=implode(',',$brand_id);
    $avr=json_decode($value['advert'],true);
    $adver_id=$avr[0]['images_class_id'];
    $sort=$value['sort'];
	//var_dump();
}
?>
        			...
        			<div class="left_add" style="border:1px">
            				<ul class="add_conent" style="margin-top: 0;">
                					<li id="brandname" class=" clearfix">
                							<input type="hidden" id="id" value="<?php echo $layout_id?>"/>
               								<label class="label_name" style="width: 150px; margin-left: -20px;"><i>*</i>分类名称：</label>
                								<select class="form-control" id="big" name="first_categorys" style=" float:left;width: 190px; margin-left: 10px; required">
													<option value="<?php echo $categorys?>"><?php echo $category?></option>
                    				...
                    				<div id="addCommodityIndex">
                        				...
                        				<div class="input-group row">
                            				<label class="label_name"  style="width: 150px; margin-left: -20px;" ><i></i>二级分类最好（6个）：</label>
                            				<input id="assoc" type="text"  name="associated" class="contact_article_str"  value="<?php echo $smallcate;?>" />
                           					<select id="choice" class="form-control"  multiple="multiple" name="second_category" style="display: none; float:left;width: 250px; margin-left: 10px;">
											<option >选择分类</option>
											<?php $second_category=CProduct::searchSmallCateByPid($categorys);
											foreach($second_category as $scg=>$scgvalue): ?>
      										<option value="<?php echo $scgvalue[id]?>"><?php echo $scgvalue['name']?></option>
    										<?php endforeach;?>
 							 				</select>
                					...
                					<div id="odiv_goods">
					                    ...
					                    <input class='contact_goods_str' id="assocs" type="text" placeholder="关联商品"value="<?php echo $goods?>" name="associateds"  required class="contact_article_strs"/>
					                 ...
					                 <li class="clearfix">
					    				...
					    				<td id="bsgoodss">
					    					<select  class="form-control bsgoods"  name="products[]"  style="float:left;width: 190px; margin-left: 10px;">
												...
												<?php $second_category=CProduct::searchproductByPid($categorys);
													foreach($second_category as $scg=>$scgvalue1): ?>
      												<option value="<?php echo $scgvalue1[cid]?>"><?php echo $scgvalue1[title]?></option>
    												<?php endforeach;?>
											</select>
										</td>
					        			<td style="padding:10px 20px">
					            			<label><input name="tr.<?php echo$vv['id']?>" type="radio" class="ace" <?php if($vv['is_new']==1){echo "checked='checked'";}?> value="1" /><span class="lbl">是</span></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					                        <label><input name="tr.<?php echo$vv['id']?>" type="radio" class="ace"  <?php if($vv['is_new']==0){echo "checked='checked'";}?> value="0" /><span class="lbl">否</span></label>
					        			</td>
					    				</tr>
					    				<?php endforeach;?>
										</table>
					                ...
					                <li class="clearfix">
					                    <div id="addCommodityIndexa">
					                        <!--input-group start-->
					                        <div class="input-group row">
					                            <label class="label_name" style="width: 150px; margin-left: -20px;"><i></i>推荐品牌最好（5个）：</label>
					                            <input id="assoca" type="text" name="associateda"  value="<?php echo $brand?>" class="contact_article_strsss"/>
												...
												<option >选择品牌</option>
												<?php $second_category=CProduct::searchblandByPid($categorys);
													foreach($second_category as $scg=>$scgvalue2): ?>
      												<option value="<?php echo $scgvalue2[bid]?>"><?php echo $scgvalue2[brandname]?></option>
    												<?php endforeach;?>
					 							 </select>
					                ...
					                <li class="clearfix">
					                	  <label class="label_name" style="width: 150px; margin-left: -20px;"><i></i>广告id：</label>
												...
												<option value="<?php echo $adver_id?>"><?php $addver=CImages::searchImage_byid($adver_id); echo $addver['name']?></option>
					                 ...
					                 <li id="sort" class=" clearfix"><label class="label_name" style="width: 150px; margin-left: -20px;"><i>*</i>排列序号：</label> <input name="sort" type="text" class="add_text"  value="<?php echo $sort;?>"/></li>
...
function addInput() {
    ...
    tr.innerHTML ="<td>" + modelsl + "</td><td style=\"padding:10px 20px\"> <label><input name=\"tr" + id + "\" type=\"radio\" class=\"ace\"  value=\"0\"><span class=\"lbl\">是</span></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label><input name=\"tr" + id + "\" type=\"radio\" class=\"ace\" checked=\"checked\" value=\"1\"><span class=\"lbl\">否</span></label></td>";
...
//              }
           ...
           $.post("/product/editlayout",
                   {
               id:id,first_categorys:first_categorys,second_categorys:second_categorys,model:model,brand:brand,adver:adver,model_new:model_new,sort:sort,choice:choice
               },
                   function(data)
                   {
                	  ...
                	  if(data==''){
      					...
      					layer.alert('修改成功',function(){window.location.href="/product/layout";});
