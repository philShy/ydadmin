    ...
    <form action="" method="post" id="iform">
        <div class="left_Competence_add">
            ...
            <div class="Competence_add">
                <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 角色名称 </label>
                    <div class="col-sm-9"><input type="text" id="form-field-1" placeholder="" value="<?php echo $auth_one_str['role']?>" name="role-name" class="col-xs-10 col-sm-5"></div>
                </div>
                <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 角色描述 </label>
                    <div class="col-sm-9"><textarea name="role-introduce" class="form-control" id="form_textarea" placeholder="" onkeyup="checkLength(this);"><?php echo $auth_one_str['introduce']?></textarea><span class="wordage">剩余字数：<span id="sy" style="color:Red;">200</span>字</span></div>
                </div>
                <!--按钮操作-->
        ...
        <div class="Assign_style">
            ...
            <?php foreach($auth_arr0 as $key=>$value):?>
                ...
                <ul>
                    <li class="li0">
                        <?php echo $value['auth_name'];?><input <?php if(in_array($value['id'],$auth_id_one_arr)){echo 'checked';}?> name="checkAll[]" class="checkAll checkAll<?php echo $value['id'];?>" type="checkbox" value="<?php echo $value['id'];?>">
                    </li>

                    <?php foreach($auth_arr1 as $key1=>$value1):?>
                        <li class="li1">&nbsp;&nbsp; &nbsp;&nbsp;<?php echo $value1['auth_name'];?><input <?php if(in_array($value1['id'],$auth_id_one_arr)){echo 'checked';}?> name="subBox[<?php echo $value['id'];?>][]" class="subBox subBox<?php echo $value['id'];?>" type="checkbox" pid="<?php echo $value1['pid'];?>" value="<?php echo $value1['id'];?>"><?php //echo $value1['id']; var_dump($auth_id_one_arr)?></li>
                    <?php endforeach;?>
                </ul>
            <?php endforeach;?>
        </div>
