    ...
    <div class="cover_style" id="cover_style">
        ...
        <div class="centent_style" id="centent_style">
            ...
            <div class="search_style">
                ...
                <a href='/transaction/orderform3'><button>交易失败订单</button></a>
            </div>
            <!--订单列表展示-->
            <table class="table_order" width="100%">
                ...
                <tr>
                    <td>   
	                    <?php if($value['is_had_order_sub_id']==1):?>
	                    <?php $sub_id_arr = CTransaction::search_ordersub_id($value['id'],$sign);?>
						<table width=100% height=100%>
						<?php foreach($sub_id_arr as $sub_k=>$sub_id):?>
						<?php $order_detail_num=CTransaction::seacrh_order_num($value['id']);?>
						
							<?php if($sub_id['goods_model_id']!=0):?>
								<tr>
									<?php if($sub_k == 0){echo "<td style='text-align:center;width:50%;border-right:1px solid #ccc' rowspan=$order_detail_num>$value[id]</td>";}?>
									<td class='orderid' style="height:44px;border-bottom:1px solid #ccc;text-align: center;"><?php echo $sub_id['id']; ?></td>
								</tr>
							<?php else:?>	
							<?php $order_detail_notnum=CTransaction::seacrh_order_notnum($sub_id['id']);?>
								<tr>
									<td rowspan='<?php echo $order_detail_notnum?>' style="text-align: center;height:44px"><?php echo $sub_id['id']; ?></td>
								</tr>
							<tr></tr>
							<tr></tr>
							<?php endif;?>	
                    ...
                    <?php if($value['is_had_order_sub_id']==0):?>
                    <?php $order_details = CTransaction::searchOrderdetail($value['id'])?>
                    <td style="border-right:none;border-left:none">
                    <?php foreach ($order_details as $goods_key=>$goods_value):?>
                      <table width="100%" cellpadding="4" cellspacing="0" border="0" style="border: none; ">
                        <tr>
                        		<td style="width:55%;">
	                                ...
	                                echo '【'.$goods_name.'】'.$goods_value['goods_model_name']; 
                                	?>
                                    </div>
                                </td>
                                <td  style="width:15%;">
              	                  <div style="width:100%;text-align:center;"><?php echo $goods_value['price']; ?></div>   
                                </td>
                                <td  style="width:15%;">
                                <div style="width:100%;text-align:center;"><?php echo $goods_value['number']?></div>
                                </td>
                              	<td  style="width:15%;">
                    ...
                    <td style="border-right:none;border-left:none">                
                        ...
                        <?php elseif($value['status'] == 2):?>
                        	...
                        	<span style='border:1px solid #ccc'>
                            	...
                            	<a class = 'receive' href = 'javascript:void(0);' type='0' order_id = '<?php echo $value['id']?>'>请接单</a>
                        	</span>             
                    ...
                    <td style="border-right:none;border-left:none;height:44px;">
                        <?php if($value['status'] != 0): ?>
                            <a href='javascript:;' type="0" orderid='<?php echo $value['id']?>' title='发货'  class='btn btn-xs btn-success Delivery_stop1'><i class='fa fa-cubes bigger-120'></i></a>
                        <?php endif;?>
                    ...
                    <!-- 如果有子订单 -->               	
                   	...
                   	<?php $sub_id_arr = CTransaction::search_ordersub_id($value['id']);?>
									...
									<?php foreach($sub_id_arr as $sub_id):?>
										...
										<?php $sum=0?>
											<tr>
														...
														<tr>
															<td width=55%;>
																<div style="width:100%;height:20px;overflow: hidden;text-align:left;margin-left:3px"> 
																	...
																	<?php 
																	$goods_name = CProduct::searchGoodsname_byid($sub_id['goods_model_id'])['name'];
		                                							echo '【'.$goods_name.'】'.$have_detail[0]['goods_model_name']; 
																	?>
																<div>
															</td>
															<td style="width:15%;">
												...
												<?php if($sub_order['status'] == 1):?>
					                            <span style="color: white;background: #438eb9">未支付</span>
						                        <?php elseif($sub_order['status'] == 2):?>
						                      
						                        	<span style='border:1px solid #ccc'>
						                        		<span class='dddd' style="color: white;background: #438eb9">已支付</span><br/>
						                            	<a class = 'receive' href = 'javascript:void(0);' type='1' order_id = '<?php echo $sub_order['id']?>'>请接单</a>
						                        	</span>             
						                        <?php elseif($sub_order['status'] == 3):?>
						                            <span style="color: white;background: #438eb9">已取消</span>
						                        <?php elseif($sub_order['status'] == 4):?>
						                        <span style="color: white;background: #438eb9">已接单</span>
						                        <?php elseif($sub_order['status'] == 5):?>
						                        <span style="color: white;background: #438eb9">已发货</span>
						                        <?php elseif($sub_order['status'] == 6):?>
						                        <span style="color: white;background: #438eb9">已收货</span>
						                        <?php elseif($sub_order['status'] == 7):?>
						                        <span style="color: white;background: #438eb9">已评价</span>    
						                        <?php endif;?>
												</td>
												<td style="width:17.8%;text-align:center;border-bottom:1px solid #ccc;height:44px;">
												<?php if($value['status'] != 0): ?>
						                            <a href='javascript:;' type="1" order_fid='<?php echo $value['id']?>' orderid='<?php echo $sub_id['id']?>' title='发货'  class='btn btn-xs btn-success Delivery_stop1'><i class='fa fa-cubes bigger-120'></i></a>
						                        <?php endif;?>
												...
												<tr>
													<td width=55% >
														<div style="width:100%;height:20px;overflow: hidden;text-align:left;margin-left:3px"> 
															...
															echo '【'.$goods_name.'】'.$have_detail_val['goods_model_name'];
															?>
														</div>
													</td>
													<td style="width:15%;">
...
<div id="Zeng_info" style=" display:none">
    <div class="">
        <div class="content_style">
            ...
            <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1">拒绝理由</label>
	...
	$(function(){
		$('.receive').click(function(){
			...
			layer.confirm('是否接单',function(index){
				$.post(url,{order_id:order_id,order_status:4,type:type,is_jiedan:1},function(data){
                    if(data){
                    	$_this.siblings('.dddd').text('已接单');
                    	$_this.remove();
                            ...
                            if($('#form-field-1').val()==""){
                                ...
                                layer.confirm('提交成功！',function(index){
                                    $.post(url,{is_send:1,order_id:orderid,logistics_num:logistics_num,send_method:send_method,shipper:shipper,boxNum:boxNum,remark:remark,consignee:consignee,isReceipt:isReceipt,type:type,order_fid:order_fid},function(data){
                                        if(data){                 
                                            $_this.parents("tr").find("span").text('已发货');
