    ...
    <div class="cover_style" id="cover_style">
        ...
        <div class="centent_style" id="centent_style">
            ...
            <div class="search_style">
                ...
                <a href='/transaction/orderform2'><button style="">待接单订单</button></a>
                ...
                <a href='/transaction/orderform7'><button style="background:#2a8bcc;width:120px;">申请取消的订单</button></a>
                                ...
                                <table width=100% height=100%>
                                    ...
                                    <tr style='height:0px;'>
                                        <td style='text-align:center;width:50%;border-right:1px solid #ccc' rowspan=<?php echo $count_all[$key]+1?>>
                                            <a style='display:inline-block;margin-bottom:5px;border:1px solid #ccc' class = 'qx bgc' href = 'javascript:void(0);' order_status = <?php echo $value['order_status']?> pay_price = <?php echo $value['pay_price']?> pay_payment = <?php echo $value['pay_payment']?> payment = <?php echo $value['payment']?> order_sub_id = '<?php echo $value['id']?>'>申请取消</a>
                                            <?php echo $value['id']?>
                                        </td>
                                    </tr>
                                    <?php foreach($value['goods_detail'] as $sub_k=>$sub_v):?>
                                        <tr>
                                            <td rowspan=<?php echo count($sub_v)-5 ?> class='orderid' style="height:50px;border-bottom:1px solid #ccc;text-align: center;"><?php echo $sub_k;?></td>
                                        </tr>
                                    <?php endforeach;?>
                                </table>
                            <?php else:?>
                                            ...
                                            <?php foreach($sub_id_arr as $sub_id):?>
                                                ...
                                                <?php if($sub_id['is_advance_order']==1)://是预购订单?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="width:17.8%;text-align:center;border-bottom:1px solid #ccc;height:50px;">
                                                            <?php if($value['status'] != 0): ?>
                                                ...
                                                <?php else:?>
                                                    <tr>
                                                        ...
                                                        <td style="width:17.8%;text-align:center;height:50px;">
                                                            <?php if($value['status'] != 0): ?>
                                                                <a href='javascript:;' type="1" order_fid='<?php echo $value['id']?>' orderid='<?php echo $sub_id['id']?>' title='发货'  class='btn btn-xs <?php if($sub_id['sub_status']==2){echo 'btn-success';}?> Delivery_stop1'><i class='fa fa-cubes bigger-120'></i></a>
                                                            <?php endif;?>
                                                            <a title="订单详细"  href="/transaction/orderdetail?id=<?php echo $sub_id['id']?>&type=1"  class="btn btn-xs btn-info order_detailed" ><i class="fa fa-list bigger-120"></i></a>
...
<!-- 取消订单-->
<div id="quxiao" style=" display:none">
    <div class="">
        <div class="content_style">
            ...
            <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"><span style="color:red">*</span> 输入金额 </label>
                <div class="col-sm-9" style="margin-top:3px;">
                    <div class="col-sm-9"><input type="text" id="refund_money" name="refund_money" placeholder="退款金额" class="col-xs-10 col-sm-5" style="margin-left:0px;"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    $(function(){
        $('.qx').click(function(){
            ...
            layer.open({
                ...
                btn:['确定','取消'],
                yes: function(index){
                    var pay_price1 = $('#refund_money').val();
                    if(pay_price1>pay_price)
                    {
                        layer.alert('error');
                        return false;
                    }
                    $.post(url,{pay_payment:pay_payment,order_id:order_id,order_status:order_status,qx:1,payment:payment,pay_price:pay_price1},function(data){
                        if(data){
