    ...
    <div class="cover_style" id="cover_style">
        ...
        <div class="centent_style" id="centent_style">
            ...
            <div class="search_style">
                ...
                <a href='/transaction/orderform5'><button style="background:#2a8bcc;">待收货订单</button></a>
            ...
            <table class="table_order" width="100%">
                ...
                <tr class='ppp'>
                            ...
                            <table width=100% height=100%>
                                ...
                                <tr style='height:0px;'>
                                    <td style='text-align:center;width:50%;border-right:1px solid #ccc' rowspan=<?php echo $count_all[$key]+1?>>
                                        <?php if($c_id['sub_status']==1):?>
                                            <a style='display:inline-block;margin-bottom:5px;border:1px solid #ccc' class = 'receive' href = 'javascript:void(0);' order_sub_id = '<?php echo $value['id']?>'>请接单</a>
                                        <?php endif;?>
                                        <?php echo $value['id']?>
                                    </td>
                                </tr>
                                <?php foreach($value['goods_detail'] as $sub_k=>$sub_v):?>
                                    <tr>
                                        <td rowspan=<?php echo count($sub_v)-5 ?> class='orderid' style="height:50px;text-align: center;"><?php echo $sub_k;?></td>
                                    </tr>
                                <?php endforeach;?>
                            </table>
                        <?php else:?>
                    ...
                    <td style="border-right:none;border-left:none">                
						...
						<?php elseif($value['status'] == 2):?>
							...
							<span style="color: white;background: #438eb9">已发货</span>
                                <br><a href="javascript:void(0)" onclick="is_recive(<?php echo $sub_id['id']?>,this)" style="color: orange;">是否收货</a>
                    ...
                    <td style="border-right:none;border-left:none;height:50px;">
                    ...
                    <!-- 已拆分 -->               	
                   	...
                   	<?php $sub_id_arr = CTransaction::search_ordersub_id($value['id'],$status=3);?>
...
<script>
    function is_recive(order_sub_id,obj) {
        var url = '/transaction/orderform';
        layer.confirm('是否确认收货',function () {
            $.post(url,{order_sub_id:order_sub_id,is_receive:1},function(data) {
                if(data)
                {
                    layer.alert('已收货');
...
$(function(){
    ...
    $(function () {
        $('.Delivery_stop1').click(function(){
        	...
        	var url = '/transaction/orderform4';
            $_this = $(this);
            var orderid = $(this).attr('orderid');
